...
[traindata] = xlsread(fullfile('C:\Users\gokkul nath\Desktop\Ramanathan sir SVM\Climate Prediction\day 2\train 15 test 14\traindata.xlsx')) ;
[testdata] = xlsread(fullfile('C:\Users\gokkul nath\Desktop\Ramanathan sir SVM\Climate Prediction\day 2\train 15 test 14\testdata.xlsx')) ;
% read the data set
...
folds = 10;
[C,gamma] = meshgrid(-5:3:15, -15:2:5);
...
for i=1:numel(gamma)
    cv_acc(i) = svmtrain(labels, vecN, ...
                    sprintf('-s 0 -t 1 -d 6  -h 0 -g %f -v %d ',  2^gamma(i), folds));
end
...
model = svmtrain(labels, data, ...
                    sprintf('-s 0 -t 1 -d 5 -c %f -g %f  -b 1', best_C, best_gamma));
